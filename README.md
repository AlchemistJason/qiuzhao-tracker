...
- 🆕 **爬虫候选池**：WorkBuddy 定时爬取校招来源写入 `discovered.json`（重点关注：互联网大厂 / 头部券商与公募基金 / 央企广东分公司，已内置 160 家核实过官网的种子公司（含阿里系 11 个子集团独立入口，按集团各 2 个投递名额）），站点自动合并进列表（行业筛选「新发现」可单独查看），设状态即开始跟踪
