...
- ✅ **待办视图**：邮件动态（笔试/面试/Offer/截止）+ 公司投递截止 + 进行中状态，统一汇总为待办清单，按 已逾期/今天/3天内/本周/更晚 分组；支持 ✓完成（沉底可恢复）、✕忽略、📅 导出 .ics 日历
...
│   └── main.js           #   启动装配
├── tests/run-tests.js    # 187 项回归测试（node tests/run-tests.js）
...
- 可用 💾 导出备份 / 📥 导入恢复
- 待办三态：动态条「采纳」= 出动态条但留在待办视图；待办视图「完成」= 沉底可 ↩恢复；「忽略」= 彻底隐藏
...
- 数据更新：改 `data.js` / `dynamics.json` → 本地测试 → 发布
- CI：push 后 GitHub Actions 自动执行语法检查 + 187 项回归测试，通过后自动部署 Pages
